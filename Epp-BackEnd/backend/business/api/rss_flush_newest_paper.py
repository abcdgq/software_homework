...
class ArxivPaperAPI(View):
    ...
    def get_newest_papers(self, request, category=None):
        ...
        # 检查分类参数
        if not category:
            return fail(msg="缺少分类参数", data={"code": 400})
        ...
        # 获取RSS数据
        parsed, error = self._fetch_rss(self.RSS_FEEDS[category])
        if error:
            return fail(msg=f"获取数据失败: {error}")

        # 解析数据
        entries = [self._parse_entry(entry, category) for entry in parsed.entries]
